...
public class MySqlUtilities implements DbUtilities{
    ...
    private String dbName = "mak213_bank1017"; // default database name
    private String dbUserName = "mak213"; // login name for the database server
    private String dbPassword = "3629398"; // password for the database server
    ...
    public MySqlUtilities(String hostName, String dbName, String dbUserName, String dbPassword) {
    	//overlaoding the constructor in case you want to connect to a different database
     ...
     * @throws SQLException
     */
    public ResultSet getResultSet(String sql) throws SQLException {  //accepts a single sql query
        try {
            if(conn == null){ // Check if connection object already exists
                createDbConnection(); // If does not exist, create new connection
            }
            Statement rs = conn.createStatement();//result statement of type statement
            return rs.executeQuery(sql); // Return ResultSet
        } catch (Exception e) {
    ...
    public boolean executeQuery(String sql){
        try {
            ...
            return true; //true if there's no error
        } catch (Exception e) {
        	e.printStackTrace(); // debug
            ErrorLogger.log(e.getMessage()); // Log error
